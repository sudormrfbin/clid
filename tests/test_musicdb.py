...
class TestMusicDB:
    ...
    def f_musicdb(self):
        ...
        db._music_files = {
            ...
            "ogg": ["/a/b/fRiEnDs.ogg", "/a/b/c/friends.ogg"],
        }
        return db
    ...
    def test_file_path_are_absolute(self, musicdb):
        ...
        assert all(
            [True if os.path.isabs(f) else False for f in musicdb.get_files("mp3")]
        )
    ...
    def test_simple_filename_search_empty_str(self, f_musicdb):
        ...
        assert expected == f_musicdb.filename_search(
            text="", ignore_case=True, fuzzy_search=False
        )
        assert expected == f_musicdb.filename_search(
            text="", ignore_case=False, fuzzy_search=False
        )
        assert expected == f_musicdb.filename_search(
            text="", ignore_case=False, fuzzy_search=True
        )

    def test_simple_filename_search_ign_case(self, f_musicdb):
        results = f_musicdb.filename_search(
            text="fri", ignore_case=True, fuzzy_search=False
        )
        expected = [
            "/a/b/d/FRIENDS.mp3",
            "/a/b/fRiEnDs.ogg",
    ...
    def test_simple_filename_search_no_ign_case(self, f_musicdb):
        results = f_musicdb.filename_search(
            text="FRI", ignore_case=False, fuzzy_search=False
        )
    ...
    def test_simple_filename_search_fuzzy(self, f_musicdb):
        results = f_musicdb.filename_search(
            text="fri", ignore_case=False, fuzzy_search=True
        )
        expected = [
            "/a/b/d/FRIENDS.mp3",
            "/a/b/fRiEnDs.ogg",
    ...
    def test_sort_by_ext(self, f_musicdb):
        ...
        assert results == expected
        # reverse = True
        results = f_musicdb.sort(
            files=f_musicdb.get_files(), sortby="ext", reverse=True
        )
        expected = expected[::-1]
        assert results == expected
    ...
    def test_sort_by_name(self, f_musicdb):
        ...
        # reverse = True
        results = f_musicdb.sort(
            files=f_musicdb.get_files(), sortby="name", reverse=True
        )
        expected = expected[::-1]
        assert results == expected
